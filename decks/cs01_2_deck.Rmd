...
We will use the NC birth data, and measure whether there is a relationship between the response, birthweight, and the explanatory variables, gestational age and biological sex.
...
## <small> Birthweight by Gestational Age (Cleaned) </small>
...
ggplot(data = birth_data, mapping = aes(x = MRACER)) +
  geom_bar() + coord_flip() + ylab("Maternal race")
```
