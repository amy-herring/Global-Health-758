...
We consider simulated data that mimic real data relevant to this question.
...
```{r m1, eval=FALSE}
library(lme4)
m1=glmer(remission ~ IL6 + CRP + CancerStage + LengthofStay + Experience + FamilyHx +  (1 | DID), family = binomial, data=lung, control=glmerControl(optimizer="bobyqa"),nAGQ=10,optCtrl=list(maxfun=2e5)) 
#these models can be flaky. Specifying a different optimizer to avoid convergence problems with default
```
...
#these models can be flaky. Specifying a different optimizer to avoid convergence
