...
With longitudinal data, we can generally make a number of assumptions about how observations are related over time to reduce the number of parameters we are estimating.
...
- Heterogeneous compound symmetry: variances may change over time but correlations remain constant: $\rho_{jk}=\rho_{j'k'}$ but we estimate $n$ different variance components $\sigma_{11}, \ldots, \sigma_{nn}$. This is a bad choice if the measurement times are not the same for all subjects (as $n$ may get pretty big)
